...
class CochlearModel:
    ...
    def solve_time_domain(self, f):
        ...
        t2 = cp.arange(0,T,self.dt/2, dtype=cp.float32)
        t = cp.arange(0,T,self.dt, dtype=cp.float32)

        alpha2 = 4*self.rho*self.b/self.dy/self.m1

        vb = cp.zeros((Ntime,Nx), dtype=cp.float32)
        ub = cp.zeros((Ntime,Nx), dtype=cp.float32)
        vt = cp.zeros((Ntime,Nx), dtype=cp.float32)
        ut = cp.zeros((Ntime,Nx), dtype=cp.float32)

        p = cp.zeros((Ntime,Nx), dtype=cp.float32)

        Ay = cp.zeros((Ny,Ny), dtype=cp.float32)

        Ay[0,0] = -2/self.dy[0]**2 - alpha2[0]
        Ay[0,1] = 2/self.dy[0]**2

        aym = cp.zeros(Ny, dtype=cp.float32)
        bym = cp.zeros(Ny, dtype=cp.float32)
        ...
        Iy = cp.eye(Ny, dtype=cp.float32)
        F = cp.zeros((Nx*Ny,Nx*Ny), dtype=cp.float32)
        ...
        k = cp.zeros(Nx*Ny,dtype=cp.float32)
...
if __name__ == "__main__":
    ...
    for fp in [1000]:
        ...
        for Lp in Lps:
            ...
            vb_cpu = vb.get()
            x = cm.x.get()
            plt.plot(x*10, 20*np.log10(np.max(np.abs(vb_cpu[int(round(vb.shape[0]*9/10)):]), axis=0)*10))
