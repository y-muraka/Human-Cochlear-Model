...
*/
  in  = (FFTW_REAL *)    malloc(dim_x *sizeof(FFTW_REAL   ));
  out = (FFTW_COMPLEX *) malloc(dim_x2 * sizeof(FFTW_COMPLEX));
  for (i = 0; i<dim_x; i++) {
    in[i] = drand48();
  }
...
//			      in, out, O_METHOD);
  ...
  plan = FFTW_PLAN_DFT_R2C_1D(dim_x,  
			      in, out, O_METHOD);
